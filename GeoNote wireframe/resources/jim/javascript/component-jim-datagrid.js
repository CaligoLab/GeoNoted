  ...
  var gridMethods = {
    ...
    "updateDataGridBounds": function() {
    	...
    	if($grid.hasClass("datagrid")){
	    	...
	    	var rowHeight = Math.max(1, Math.floor((100-percentageVerticalSpacing)/numRows));
            ...
            $grid.removeAttr("datasizewidth");
            $grid.removeAttr("datasizeheight");
    	}
	 },
     "updateDataListBounds": function() {
    	...
    	if(!$("#simulation").hasClass("blockRefresh") || dmOriginalRows != ($rows.length - 1)){
			...
			$list.removeAttr("datasizewidth");
            $list.removeAttr("datasizeheight");
    	}
	  }
  };
