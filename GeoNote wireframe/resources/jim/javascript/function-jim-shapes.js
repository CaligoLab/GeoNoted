...
(function(window, undefined) {
    ...
    "createSVGShape" : function($shape) {
        ...
        var mainDiv = $mainDiv[0];
            ...
            var wrapperWidth = parseFloat($mainDiv.attr("datasizewidthpx"));
            var wrapperHeight = parseFloat($mainDiv.attr("datasizeheightpx"));
            ...
            shapeSVG.svg.style["height"] = wrapperHeight;
                ...
                shapeSVG.css2svg = {
                    "border-width": parseFloat($(shapeSVG).css("stroke-width")) / 2,
                ...
                var style = undefined;
                if(!$shape.is(".line")){
                  var hasText = this.containsText(shapeSVG);
                  style = {
                      "attributes" : {
                          "width" : $(mainDiv).is(".autofit") ? "auto" : parseFloat(shapeSVG.css2svg["original-width"]),
                          "min-height" : $(mainDiv).is(".autofit") ? 0 : hasText ? parseFloat($(shapeSVG).css("min-height")):parseFloat(shapeSVG.css2svg["original-height"])
                  }};
                }else {
                  shapeSVG.svg.style["height"] = parseFloat(shapeSVG.css2svg["original-height"]);
                }
        ...
        "translateCSSChanges" : function(shapeSVG, style) {
            ...
            if (style.attributes) {
                if (style.attributes['stroke-width'] != null) {
                    this._setBorderWidth(shapeSVG, parseFloat(style.attributes['stroke-width']));
                }
                ...
                if (style.attributes['width'] != null || style.attributes['min-height'] != null) {
                  ...
                  this._setSize(shapeSVG, parseFloat(style.attributes['width']), parseFloat(style.attributes['min-height']));
                  ...
                  var minTextHeight = parseFloat($(shapeSVG).shapeStyle("min-height")) - parseInt($(shapeSVG).shapeStyle("padding-top")) - parseInt($(shapeSVG).shapeStyle("padding-bottom")) - nonTextboxSize.height -(borderTextPadding*2);
            ...
            if (style.expressions) {
                if (style.expressions['width'] != null && style.expressions['height'] != null) {
                    newWidth = (isNaN(parseFloat(style.expressions['width'], 10))) ? eval(style.expressions['width']) : style.expressions['width'];
                    newHeight = (isNaN(parseFloat(style.expressions['height'], 10))) ? eval(style.expressions['height']) : style.expressions['height'];
                  ...
                  //  this._setTextBoxBounds(shapeSVG,{'width':$(shapeSVG).shapeStyle("width"), 'height':$(shapeSVG).shapeStyle("height")},{'width':newWidth, 'height':newHeight});
                    this._setSize(shapeSVG, parseFloat(newWidth), parseFloat(newHeight));
                }
            }
        },
    ...
    "updateStyle" : function(shapeSVG, style, shapePath, init) {
            if (style) {
                ...
                var oldFullTextBoxSize;
            	  if(init){
                    ...
                    oldFullTextBoxSize = {
                       "width" : parseFloat($paddingLayer.css("width"))+ parseInt(shapeSVG.css2svg['original-padding-left']) + parseInt(shapeSVG.css2svg['original-padding-right']) +(this.getBorderTextPadding(shapeSVG)*2),
                       "height" : parseFloat($paddingLayer.css("height"))+ parseInt(shapeSVG.css2svg['original-padding-top']) + parseInt(shapeSVG.css2svg['original-padding-bottom']) +(this.getBorderTextPadding(shapeSVG)*2)
                   };
            	  }
                ...
                if (style.attributes) {
                    ...
                    if(style.attributes["stroke-width"]){
                        $(shapeSVG).css("stroke-width",parseFloat(style.attributes["stroke-width"]));
                        shapeSVG.css2svg['border-width'] = parseFloat(style.attributes["stroke-width"])/2;
                    }
        ...
        "updateMargin" : function(shapeSVG) {
        	...
        	$(shapeSVG.svg).css("margin","");
        	 ...
        	 // Update svg
        // 	SVGEngine._setComputedSize(shapeSVG,parseFloat($(shapeSVG.shapewrapper).css("width")) ,parseFloat($(shapeSVG.shapewrapper).css("height")));
        },
        "updateBackground" : function(shapeSVG) {
            ...
            if (backgroundImg && backgroundImg.match('url') != null){
                ...
                var shapeSize = {
                	width : parseFloat($(shapeSVG).shapeStyle("width")),
                    height : parseFloat($(shapeSVG).shapeStyle("height"))
                };
            ...
            if(backgroundGradient && backgroundGradient.match('gradient') != null){
				...
				if (backgroundGradient.match("linear-gradient") || backgroundGradient.match("radial-gradient")){
	                ...
	                defsContent = this._getStandardGradient(backgroundGradient, parseFloat($(shapeSVG).shapeStyle("width")), parseFloat($(shapeSVG).shapeStyle("height")));
	                defsContent.setAttribute("id", "svg-fill-gradient-"+ $(shapeSVG).prop("id") + random4Id);
	             }
	             if(patternContainer){
	            	 patternContainer.appendChild(defsContent);
        ...
        "getBorderTextPadding" : function(shapeSVG) {
          if(shapeSVG.shapeType==="callout")
            return parseFloat($(shapeSVG).shapeStyle("border-width"));
          return 0;
        },
        ...
        "calculateTextPrefSize" : function(shapeSVG, shapeNewWidth) {
            ...
            if($(shapeSVG.shapewrapper).is(".autofit")) {
              ...
              var nonTextboxSize = this.getNonTextboxSize(shapeSVG); //callout tail size  (0 for other shapes)
              textWidth = parseFloat(shapeNewWidth) - parseInt($(shapeSVG).shapeStyle("padding-left")) - parseInt($(shapeSVG).shapeStyle("padding-right")) - nonTextboxSize.width - (this.getBorderTextPadding(shapeSVG)*2);
            }
            var $paddingLayer = $(shapeSVG.shapewrapper).children(".paddingLayer");
        ...
        //  return parseFloat($content.css("width"));
          ...
          for(t=0, tLen=spans.length; t<tLen; t+=1) {
           ...
           var bounds = span.getBoundingClientRect();
            maxX = Math.max(maxX,parseFloat(bounds.right));
            if(t==0)
              minX = parseFloat(bounds.left);
            else
              minX = Math.min(minX,parseFloat(bounds.left));
          }
        ...
        "calculateTextPrefHeight" : function(shapeSVG) {
          ...
          for(t=0, tLen=spans.length; t<tLen; t+=1) {
            span = spans[t];
            var bounds = span.getBoundingClientRect();
            var spanBottom = span.innerHTML!="" ? parseFloat(bounds.bottom):0;
            
            maxY = Math.max(maxY,spanBottom);
            if(t==0) {
              minY = parseFloat(bounds.top);
			  maxY = spanBottom;
            } else
              minY = Math.min(minY,parseFloat(bounds.top));
          }
          return maxY-minY;
        },
         "containsText" : function(shapeSVG) {
          var t,tLen,spans,span;
          var $content = $(shapeSVG.shapewrapper).find(".content");
          var spans = $content.find("span");
          for(t=0, tLen=spans.length; t<tLen; t+=1) {
      ...
      /*  "_setTextBoxBounds" : function(shapeSVG,  newSize){
        ...
        "updateShape" : function(shapeSVG, shapePath, oldSize) {
         ...
         "_updateMarkers" : function(shapeSVG) {
			...
			if(startMarker || endMarker){
				 borderWidth = parseFloat($(shapeSVG).shapeStyle("stroke-width"));
			}
            var markerWidth;
            //scale markers
			if(startMarker){
	            ...
	            if($startMarker.length != 0){
	                ...
	                $startMarker[0].setAttribute("refX", parseFloat((100 * (markerWidth - borderWidth)) / markerWidth));
			...
			if(endMarker){
	            ...
	            if($endMarker.length != 0){
	                ...
	                $endMarker[0].setAttribute("refX",parseFloat((100 * (markerWidth - borderWidth)) / markerWidth));
         ...
         "_getLinePathClipped" : function(shapeSVG, path) {
             var width = parseFloat($(shapeSVG).shapeStyle("width")),
             height = parseFloat($(shapeSVG).shapeStyle("height")),
             lineWidth = parseFloat($(shapeSVG).shapeStyle("stroke-width")),
             leftClipping = 0,
             rightClipping = width;
			...
			if($(shapeSVG).is(".eMarker")){
              ...
              return "M "+ leftClipping + " " + parseFloat(height/2) +" L "+ rightClipping + " " + parseFloat(height/2);
         },
        ...
        "_getLineHeight" : function(shapeSVG) {
            ...
            var borderWidth = parseFloat($(shapeSVG).shapeStyle("stroke-width"));
            var $startMarker = $(shapeSVG.svg).children("defs").children('[id^=start-marker]');
            var $endMarker = $(shapeSVG.svg).children("defs").children('[id^=end-marker]');
        ...
        "_getMarkerHeight" : function(marker,borderWidth) {
            ...
            if(markerType=== "open"){
                ...
                height = parseFloat((triangleHeight * Math.tan(Math.PI / 6)) * 2);
                // add arcs extra size
                height += 2 * (parseFloat((borderWidth / 2) - ((borderWidth / 2) * Math.sin(Math.PI / 3)))) + 3;
            }
        ...
        "_getScaledPath" : function(path, oldSize, newSize, closedPath) {
            ...
            (pointM[0] == 0) ? scaledPoint.x = 0 : scaledPoint.x = parseFloat((newSize.width * pointM[0]) / oldSize.width);
            (pointM[1] == 0) ? scaledPoint.y = 0 : scaledPoint.y = parseFloat((newSize.height * pointM[1]) / oldSize.height);
            scaledPath = "M " + scaledPoint.x + " " + scaledPoint.y;

            for (var i = 0; i < pointsL.length; i++) {
                   var point = pointsL[i].split(" ");
                   (point[0] == 0) ? scaledPoint.x = 0 : scaledPoint.x = parseFloat((newSize.width * point[0]) / oldSize.width);
                   (point[1] == 0) ? scaledPoint.y = 0 : scaledPoint.y = parseFloat((newSize.height * point[1]) / oldSize.height);
