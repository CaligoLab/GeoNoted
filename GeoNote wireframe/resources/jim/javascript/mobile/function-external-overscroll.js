...
//		}
	...
	moveThumbs = function (thumbs, sizing, left, top, target, notify) {
		var ml, mt;
		...
		if (thumbs && thumbs.added) {
			...
			if((thumbs.horizontal || thumbs.vertical) && notify){
	...
	scroll = function (event) {
		...
		if (!data.flags.dragged) {
			moveThumbs(data.thumbs, data.sizing, this.scrollLeft, this.scrollTop, data.target,true);
		}
	},
	...
	wheel = function (event) {
		...
		moveThumbs(thumbs, sizing, this.scrollLeft, this.scrollTop, data.target,true);
...
//		event.stopPropagation();
		...
		var newContainerSizing = getSizing(target, data.options, data.sizing).container;
		sizing.container.width = newContainerSizing.width;
		sizing.container.height = newContainerSizing.height;
		...
		moveThumbs(thumbs, sizing, target.scrollLeft, target.scrollTop, data.target,true);
...
//		target.css('cursor', compat.cursorGrab);
	...
	getThumbCss = function(size, options) {
		return {
			...
			'border-radius': options.roundCorners ? size.corner + 'px' : '0px',
	...
	setup = function(target, options) {
		...
		settings.thumbThickness = options.thickness;
		settings.thumbOpacity = options.opacity;
...
//				,cursor: compat.cursorGrab
			...
			if (thumbs.added) {
				moveThumbs(thumbs, sizing, target.scrollLeft(), target.scrollTop(), data.target, false);
