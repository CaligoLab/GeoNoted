...
(function(window, undefined) {
      ...
      "getHtml": function(type, values, $component) {
        ...
        switch(type) {
          ...
          case itemType.checkboxlist:
            isHorizontal = $component.hasClass("horizontal");
            ...
            for(i = 0; i < values.length; i += 1) {
              ...
              if(jimDevice.isMobile() && jimDevice.isIOS()) {
            	  inputDevice = "inputIOS";
              }
              html += "<td><div class=\""+type+" unchecked "+inputDevice+"\" "+property+" "+readonly+" tabindex=\"-1\"><div class=\"backgroundLayer\"><div class=\"colorLayer\" /><div class=\"imageLayer\" /></div></div><span class=\"option\">"+option+"</span></td>";
  ...
  function fireKeyboardEvents(eventType, data, type) {
	...
	if(!(data && data.preventTrigger)) { /* prevent infinite loop */
      ...
      $firer.trigger(eventType + ".jim", [{"preventTrigger": true, "altKey":event.altKey, "ctrlKey":event.ctrlKey, "shiftKey": event.shiftKey, "which":event.which}]);
    }
  }
  ...
  else if(jimUtil.isiOSDevice()) {
  	jQuery("html").bind("keydown keypressed", function(event, data) {
  		if(event.which === 13) { // ENTER
	      fireKeyboardEvents(event, data, "keyup");
	    }
  	});
  }
  ...
  var saveMousePosition = function(e){ 
	  var simulationBounds = jQuery("#simulation")[0].getBoundingClientRect();
	   ...
	   if(e.type == 'touchstart' || e.type == 'touchmove' || e.type == 'touchend' || e.type == 'touchcancel'){
        ...
        mousePos.x = (touch.pageX)-simulationBounds.left + jimEvent.fn.jimWindowScrollX(); 
	  	mousePos.y = (touch.pageY)-simulationBounds.top + jimEvent.fn.jimWindowScrollY();
      }else{
  ...
  if(document.addEventListener){
	  document.addEventListener('mousemove',saveMousePosition , false);
	   if(jimUtil.isMobileDevice()){
	  	document.addEventListener('touchstart',saveMousePosition , false);
	  	document.addEventListener('touchmove',saveMousePosition , false);
	  	document.addEventListener('touchend',saveMousePosition , false);
	  	document.addEventListener('touchcancel',saveMousePosition , false);
	  }
  }else{//IE8
