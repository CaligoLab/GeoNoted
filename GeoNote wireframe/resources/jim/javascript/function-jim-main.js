...
(function (window, undefined) {
  ...
  function navigate(target, args, forced) {
    ...
    if(target) {
	  ...
	  if(options.isexternal){
	 	if((!$('body').hasClass("onsite") && !$('body').hasClass("offline")) || $sidepanel.hasClass("hidden")){
  ...
  function setupDevice($page,settings){
	  ...
	  if((deviceName !== oldDeviceName) || ((jimDevice.isAndroid() || jimDevice.isCustom()) &&($.inArray(deviceSize,deviceClasses))<0)){
		  ...
		  //remove old classes
		  $body.removeClass("devMobile devIOS devAndroid devAndroidPhone devAndroidTablet devCustom iphone-device ipad-device android-device");
		  var cName;
		  if(deviceClasses){
		  ...
		  if(jimUtil.isMobileDevice()){
			       ...
			       else if(jimDevice.isAndroidPhone() || jimDevice.isAndroidTablet() || jimDevice.isCustom()) {
			         var deviceWidth = $page.attr("devicewidth");
			         var deviceHeight = $page.attr("deviceheight");
			         var deviceSize = "s"+deviceWidth+"x"+deviceHeight;
