...
(function(window, undefined) {
  jQuery.extend(jimEvent.fn, {
      ...
      if(jimUtil.exists(parameters)) {
        for(i=0, iLen=parameters.length; i<iLen; i+=1) {
          ...
          if(jimEvent.isNumber(parameter)) {
            ...
            value = jimEvent.fn.floatify(value);
          } else {
            value = null;
            break;
          }
    ...
    "jimPow": function(parameters, obj) {
      var self = this, value = null, base, exp;
      if(jimUtil.exists(parameters)) {
      	base = jimEvent.tryNumberConversion(self.evaluateExpression(parameters[0], obj));
      	exp = jimEvent.tryNumberConversion(self.evaluateExpression(parameters[1], obj));
        if(jimEvent.isNumber(base) && jimEvent.isNumber(exp)) {
          value = Math.pow(base, exp);
        }
      }
      return value;
    },
    "jimFloor": function(parameters, obj) {
      var self = this, value = null, valueToFloor;
      if(jimUtil.exists(parameters)) {
      	valueToFloor = jimEvent.tryNumberConversion(self.evaluateExpression(parameters[0], obj));
        if(jimEvent.isNumber(valueToFloor)) {
          value = Math.floor(valueToFloor);
        }
      }
      return value;
    },
    "jimCeil": function(parameters, obj) {
      var self = this, value = null, valueToCeil;
      if(jimUtil.exists(parameters)) {
        valueToCeil = jimEvent.tryNumberConversion(self.evaluateExpression(parameters[0], obj));
        if(jimEvent.isNumber(valueToCeil)) {
          value = Math.ceil(valueToCeil);
        }
      }
      return value;
    },
    ...
    "jimGreater": function(parameters, obj) {
      ...
      if(jimUtil.exists(parameters[0]) && jimUtil.exists(parameters[1])) {
        ...
        if((jimEvent.isNumber(op1) || jimEvent.isText(op1)) && (jimEvent.isNumber(op2)|| jimEvent.isText(op2))) {
          value = op1 > op2;
        }
      }
      return value;
    ...
    "jimGetArea":function($target) {
	    ...
	    if($target.hasClass("shapewrapper")){
	    	if($target.find('path').length>0 && $target.find('path')[0].className.baseVal.indexOf('triangle') > -1){
		    	var vertexOffset = parseFloat($target.find('path').attr('d').split(" ")[1]);
	    		...
	    		var points =[];				
				var pointString = $target.find('path').attr('d').replaceAll(/[a-zA-Z]/g, "").replaceAll(/\s\s+/g," ").trim().split(" ");
				for (var k = 0; k < pointString.length; k+=2)
					points.push({x: parseFloat(pointString[k]), y: parseFloat(pointString[k + 1])});
